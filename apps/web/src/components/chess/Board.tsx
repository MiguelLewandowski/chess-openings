            ...
            if (visualMarkers) {
                ...
                if (visualMarkers.arrows) {
                    visualMarkers.arrows.forEach((arrow: string) => {
                        if (arrow.length === 5) {
                            shapes.push({
                                orig: arrow.substring(1, 3) as Key,
                                dest: arrow.substring(3, 5) as Key,
                                brush: getBrushColor(arrow[0])
                            });
                        } else if (arrow.length === 4) {
                            shapes.push({
                                orig: arrow.substring(0, 2) as Key,
                                dest: arrow.substring(2, 4) as Key,
                                brush: 'green'
                            });
                        }
                    });
                ...
                if (visualMarkers.circles) {
                    visualMarkers.circles.forEach((circle: string) => {
                        if (circle.length === 3) {
                            shapes.push({
                                orig: circle.substring(1, 3) as Key,
                                brush: getBrushColor(circle[0])
                            });
                        } else if (circle.length === 2) {
                            shapes.push({
                                orig: circle.substring(0, 2) as Key,
                                brush: 'green'
                            });
                        }
                    });
